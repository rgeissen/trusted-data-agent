...
async def list_chart_mcp_tools(server_http_url: str):
    """
    Connects to the specified chart MCP server using Streamable HTTP transport
    ...
    """
    print(f"Attempting to connect to Chart MCP server at: {server_http_url} using Streamable HTTP transport.")

    # Define the server configuration for MultiServerMCPClient
    server_configs = {
        'chart_mcp_server': {
            "url": server_http_url,
            "transport": "streamable_http" # Corrected transport to "streamable_http"
        }
    }
...
if __name__ == "__main__":
    # The HTTP URL of your chart MCP server for Streamable HTTP transport
    CHART_MCP_SERVER_HTTP_URL = "http://localhost:1122/mcp"
